...
local attach = function(client, bufnr, alt_keys)
    ...
    vim.keymap.set("n", "[d",         alt_keys and alt_keys.diagnostic_goto_prev  or old_goto_prev,                   opts)
    vim.keymap.set("n", "]d",         alt_keys and alt_keys.diagnostic_goto_next  or old_goto_next,                   opts)
    ...
    if vim.iter({ "omnisharp", "gopls" }):all(is_stoopid) then
