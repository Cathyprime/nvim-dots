return {
    -- lspconfig
    {
        ...
        config = function(_, opts)
            local Util = require("lsp-setup.util")
            ...
            -- setup autoformat
            require("lsp-setup.plugins.lsp.format").setup(opts)
            -- setup formatting and keymaps
            Util.on_attach(function(client, buffer)
                require("lsp-setup.plugins.lsp.keymaps").on_attach(
                    client,
                    buffer
                )
            end)
            ...
            -- diagnostics
            for name, icon in
                pairs(require("lsp-setup.config").icons.diagnostics)
            do
            ...
            then
                opts.diagnostics.virtual_text.prefix = vim.fn.has("nvim-0.10.0")
                    ...
                    or function(diagnostic)
                        local icons =
                            require("lsp-setup.config").icons.diagnostics
            ...
            if
                Util.lsp_get_config("denols")
                and Util.lsp_get_config("tsserver")
            then
